...
class Actor(BaseActor):
    def __init__(self, obs_dim, act_dim, action_bound, noise_scale, hidden_dims, activation_fc_name):
        ...
        self.register_buffer("action_rescale",
             torch.as_tensor((action_bound[1] - action_bound[0]) / 2., dtype=torch.float32))
        self.register_buffer("action_rescale_bias",
            torch.as_tensor((action_bound[1] + action_bound[0]) / 2., dtype=torch.float32))
        self.apply(weight_init)
...
class DDPGONNXPolicy(BaseActor):
    def __init__(self, obs_dim, act_dim, action_bound, hidden_dims, activation_fc_name):
        ...
        action_rescale = torch.as_tensor((action_bound[1] - action_bound[0]) / 2., dtype=torch.float32)
        action_rescale_bias = torch.as_tensor((action_bound[1] + action_bound[0]) / 2., dtype=torch.float32)
        self.register_buffer('action_rescale', action_rescale)
        self.register_buffer('action_rescale_bias', action_rescale_bias)
