...
class TQCONNXPolicy(BaseActor):
    def __init__(self, state_dim, action_dim, hidden_dims, action_bound, activation_fc_name):
        super(TQCONNXPolicy, self).__init__(activation_fc_name)
    ...
    def forward(self, state):
        ...
        for i, hidden_layer in enumerate(self.hidden_layers):
            x = self.activation_fc(hidden_layer(x))
        mean = self.mean_layer(x)
        action = torch.tanh(mean) * self.action_rescale + self.action_rescale_bias
        return action